...
class RequestManagerDb {
  ...
  async _processInput() {
    ...
    for (const file of inputPaths) {
      ...
      for await (const line of rl) {
        this.db.prepare('INSERT INTO requests (inn, status) VALUES (?, ?)').run(line, 'raw');
      }
  ...
  _updateAfterRequest(success, validationErrors, requestErrors, stopErrors) {
    ...
    console.log(success);
    success.forEach((item) => updateStatus.run('success', item));
...
new RequestManagerDb({ dbFile: 'pdf.db' }).start();
